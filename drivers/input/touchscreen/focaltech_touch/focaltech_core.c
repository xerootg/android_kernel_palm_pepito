...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    do {
        ...
        if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
            FTS_ERROR("i2c read error, read:0x%02x%02x", chip_id[0], chip_id[1]);
        } else {
            ret = fts_get_chip_types(ts_data, chip_id[0], chip_id[1], VALID);
            if (!ret)
                break;
            else
                FTS_ERROR("TP not ready, read:0x%02x%02x", chip_id[0], chip_id[1]);
        }
...
static ssize_t tct_tp_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    /* Add-Begin-By falin.luo for tell FT8613 and FT8716 apart */
    #if 0
	num_read_chars = snprintf(buf, 128, "%s_%s_V%02X\n", "FT8716", "TDT", fwver);
    #else
    if (ts_data->ic_info.ids.type == 0x0C) {
        num_read_chars = snprintf(buf, 128, "%s_%s_V%02X\n",
                "FT8613", "TDT", fwver);
    } else if (ts_data->ic_info.ids.type == 0x05) {
        num_read_chars = snprintf(buf, 128, "%s_%s_V%02X\n",
                "FT8716", "TDT", fwver);
    } else {
        num_read_chars = snprintf(buf, 128, "%s_%s_V%02X\n",
    ...
    #endif
    /* Add-Begin-End falin.luo for tell FT8613 and FT8716 apart */
...
static ssize_t tct_tp_version_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	
	return count;
}

static DEVICE_ATTR(tp_version,0664, tct_tp_version_show, tct_tp_version_store);

static struct attribute *tct_tp_attr_list[] =
{
	&dev_attr_tp_version.attr,
	NULL,
};

static struct attribute_group tct_tp_attr_group =
{
    .attrs = tct_tp_attr_list,
};
// Add-end by ning.wei
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	// add by ning.wei start
	ts_data->tct_tp_dev = platform_device_register_simple("tct_tp", -1, NULL, 0);
	if (IS_ERR(ts_data->tct_tp_dev))
	{
		printk("%s:Can not create node at /sys/platform\n", __func__);
	}	

	ret = sysfs_create_group(&ts_data->tct_tp_dev->dev.kobj, &tct_tp_attr_group);
	if (ret !=0)
	{
...
static int __init fts_ts_init(void)
{
    ...
    if ( ret != 0 ) { // MODIFIED by junwen.ye, 2017-11-24,BUG-5653823
